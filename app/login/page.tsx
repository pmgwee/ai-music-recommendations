...
/**
 * Email + password sign-in. On success the Supabase browser client sets the
 * auth cookies and we navigate to the sanitized `next` (defaulting to
 * /dashboard). Google OAuth calls signInWithOAuth with a redirectTo back to the
 * PKCE callback — it'll 400/no-op until the user enables Google in Supabase
 * Auth settings, which is fine.
 *
 * Query params are read from `window.location` (not `useSearchParams`) so the
 * page stays a single client component without a Suspense boundary.
 */
export default function LoginPage() {
  ...
  // Read `?next` and `?error` once on mount (client-only).
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const n = params.get("next");
    if (n) setNext(n);
    if (params.get("error") === "callback") {
      setError("That sign-in link is invalid or expired. Please try again.");
    }
  }, []);
